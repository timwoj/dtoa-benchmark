...
// static constexpr double _pow10(int prec) { return pow(10, prec); }
// static constexpr double _pow10r(int prec) { return pow(0.1, prec); }
...
static size_t sn_strip_trailing_zeros(char* str) {
  ...
  for (;;) {
    ...
    if (*frac == '0') {
      if (!trailing_zeros) trailing_zeros = frac;
    } else
      trailing_zeros = 0;
  ...
  if (trailing_zeros == start_dec) --trailing_zeros;

  if (!trailing_zeros || !exp) return 0;
...
size_t modp_litoa10(int64_t value, char* str) {
  char* wstr = str;
  uint64_t uvalue =
      (value < 0) ? (value == INT64_MIN ? (uint64_t)(INT64_MAX) + 1 : -value)
                  : value;
